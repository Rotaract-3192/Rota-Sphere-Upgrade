import { Skeleton, EventCardSkeleton } from "@/components/ui/LoadingSkeleton";

export default function EventsLoading() {
  return (
    <main className="min-h-screen bg-gray-50 dark:bg-gray-950 pb-20 animate-fade-in" aria-busy="true">
      {/* ── HERO DISCOVERY BANNER SKELETON ── */}
      <section className="bg-gray-900 text-white py-12 sm:py-16 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
        <div className="max-w-7xl mx-auto space-y-6 relative z-10">
          <div className="space-y-3 max-w-2xl">
            <Skeleton className="w-56 h-4 rounded-full" />
            <Skeleton className="w-full sm:w-4/5 h-10 sm:h-14 rounded-2xl" />
            <Skeleton className="w-full sm:w-3/4 h-4 rounded-md" />
          </div>

          {/* Search Bar Skeleton */}
          <div className="max-w-2xl flex gap-2">
            <Skeleton className="flex-1 h-12 rounded-2xl" />
            <Skeleton className="w-28 h-12 rounded-2xl" />
          </div>

          {/* Category Filter Pills Skeleton */}
          <div className="flex items-center gap-2 overflow-x-hidden pt-1">
            {Array.from({ length: 6 }).map((_, i) => (
              <Skeleton key={i} className="w-24 sm:w-28 h-8 rounded-full shrink-0" />
            ))}
          </div>
        </div>
      </section>

      {/* ── EVENT LISTINGS SKELETON ── */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 space-y-8">
        {/* View Switcher Header Bar */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-200 dark:border-gray-800 pb-4">
          <div className="space-y-1.5">
            <Skeleton className="w-48 h-6 rounded-lg" />
            <Skeleton className="w-40 h-3.5 rounded-md" />
          </div>
          <div className="flex items-center gap-2">
            <Skeleton className="w-28 h-10 rounded-2xl" />
            <Skeleton className="w-36 h-10 rounded-2xl" />
          </div>
        </div>

        {/* 4-Column Grid Skeleton */}
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {Array.from({ length: 8 }).map((_, i) => (
            <EventCardSkeleton key={i} />
          ))}
        </div>
      </section>
    </main>
  );
}
