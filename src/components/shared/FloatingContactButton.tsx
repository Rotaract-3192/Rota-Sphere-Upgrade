...
export function FloatingContactButton() {
  ...
  return (
    <>
      ...
      {open && (
        <div
          className="fixed inset-0 z-35 bg-black/30 dark:bg-black/60 backdrop-blur-[1px] md:hidden animate-in fade-in duration-150"
      ...
      <div className="fixed bottom-20 md:bottom-6 left-4 md:left-auto md:right-6 z-40 flex flex-col items-start md:items-end">
        {/* Popover Menu */}
        {open && (
          <div className="mb-3 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-3xl p-4 shadow-2xl text-gray-900 dark:text-white space-y-3 w-72 max-w-[calc(100vw-32px)] max-h-[75vh] overflow-y-auto animate-in fade-in slide-in-from-bottom-2 duration-200">
        ...
        <button
          ...
          data-tour="floating-guide-btn"
          className="px-3.5 py-2.5 sm:p-3.5 rounded-2xl bg-[#0758fc] hover:bg-[#054fe0] text-white shadow-xl hover:scale-105 active:scale-95 transition-all duration-200 cursor-pointer flex items-center gap-2 font-bold text-xs group"
        >
          <MessageCircle size={18} className="group-hover:rotate-12 transition-transform shrink-0" />
          <span className="font-extrabold sm:hidden">Guide</span>
